...
Any additional clarifications I add are marked with **(Added clarification: …)**.

---

# **2. Configuring Queue Connections — Documentation**
...
⚠️ Removes even jobs that haven't started yet.
...
### **• Some connections require migrations**

(database, failed-jobs)

### **• Redis / SQS / Beanstalkd do not**

### **• Many artisan commands exist to inspect, monitor, clear, prune, and restart queues**
...
Here’s the explanation **added to your documentation style**, clear and concise:
...
(Added clarification: That’s why in production you must run workers using **Supervisor**, **systemd**, **pm2**, or Horizon — so they stay alive even after server restarts.)
...
Because PHP can’t run long processes by default, we use:
...
* Batch jobs won’t execute
* Failed jobs can't retry
* The queue system becomes useless

Workers are the “engine” that processes everything placed into the queue.